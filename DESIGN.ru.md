...
Правило 2 — обещание про churn, а значит, его долю пула надо отложить
**до** того, как её потратит переиспользование. Обе половины берут из
одного пула, и переиспользование идёт первым: `take` изымает слот, а
churn потом выбирает из того, что осталось. Коммит, вольный
переиспользовать весь пул, не оставил бы для выборки ничего, а
`sample_distinct` возвращает `n.min(len)` — он молча урезает, — и
коммит сохранил бы правило 2 на словах, нарушив его на диске.

Поэтому эпизод записи вправе переиспользовать не более

```
pool / (1 + CHURN_PER_REUSE)
```

слотов, а остаток оставляет на финансирование их churn
(`reuse_floor_for`). Сверх бюджета он дописывает в конец: это плата
ростом файла и никогда — отрицаемостью, а только в эту сторону такой
размен и вправе ломаться. При `CHURN_PER_REUSE = 1` выходит ровно
пополам — то же деление, которое правило 2 описывает на диске. Пул из
одного слота не финансирует ничего и не переиспользует ничего, а
контейнер сразу после первого `vacuum_orphans` — как раз этот случай.

Бюджет тратится там, где слот покидает пул, а не решается один раз на
коммит. `publish_superblock` читает *эпохальную* половину предиката
переиспользования однажды, до сжигания seq, и отдаёт один ответ всем
репликам — верно для вопроса об эпохе и неверно для ресурса, который
размещения расходуют по ходу дела.

У правила 1 — парное следствие: churn выполняется только в `commit_tx`,
значит только там допустимо и переиспользование.
`write_self_heal_checkpoint` тоже публикует суперблок, и слоты под него
он берёт дописыванием, а не из пула, потому что переиспользованный слот
здесь — это слот, который никакой churn не прикроет.
...
v0.1. Все пять имеют отгруженные resolution'ы.

Эта преамбула говорила, что item 3 — «soft cap, документированный в
threat-model'е как out of scope», тогда как сам item 3 тремя абзацами ниже
описывает hard cap, который проверяется и на write-, и на read-стороне. Тот
же дрейф, о котором пишет сам item, уровнем выше (report17 HV17-L2).
