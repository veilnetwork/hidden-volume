...
теперь рабочий набор ограничен размером самого namespace. Со времён
аудита HV-16 это цена только `repack`: обычный `Tx::commit` namespace
больше не материализует, он спускается к затронутому листу и
переписывает путь над ним (`space::tree`). См.
