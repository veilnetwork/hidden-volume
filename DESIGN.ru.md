...
(~48 B). Замерено сквозным тестом `tests/open_peak_memory.rs`: **0,84
байта пика кучи на владеемый слот**, то есть ~13 МиБ на потолке в 16M
слотов при планке 2,0. (Здесь стояло 0,16 — до report27 H07: та же
библиотека, но замеренная на паре фикстур, между которыми слишком мало
слотов, чтобы что-то увидеть. Изменилось число, а не библиотека.) Было 27,5 Б/слот — 440 МиБ на потолке, — пока `owned_slots`
...
3. **Maximum slot count.** ✅ Решено. При `u64` seq и 4 KiB chunks файл
   доходит до 64 EiB, а настоящим пределом была память при scan — поэтому
   `MAX_OPEN_SCAN_CHUNKS` (§10) enforce'ится с ОБЕИХ сторон: write-путь
   отказывается расти за него, каждый open-scan путь отказывается за него
   сканировать. Эта запись говорила, что библиотека не enforce'ит жёсткий
   cap, тогда как §10 того же документа его описывал — дрейф, который
   читатель разрешает в ту сторону, что дороже ему обойдётся (report16
   HV16-L1). Практическое руководство по-прежнему в
