...
**Статус.** **Заморожен**, и заморожен с v1.0.0 (2026-05-28) — см.
[`docs/ru/guide/migration.md`](../guide/migration.md). Любое дальнейшее
изменение раскладки требует нового поколения v4 и инструмента миграции.
Мажорный бамп 2.0.0 был взят ради публичных API (Rust / FFI / Dart) и
поколение формата **не** сдвинул; `CHANGELOG.md` называет то единственное
направление совместимости контейнеров, которое релиз не сохраняет.
...
| биты   | поле                       | семантика                                                                                                                                                                                                                                                                  |
|--------|----------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 0..16  | `format_version`           | Сейчас `3` (v3 cluster: #8 kind-tag bytes + #9 cryptographic version-binding + #10 per-space derived `container_id`). Библиотека отказывается открывать при любом другом значении. v1/v2 контейнеры не читаются v3-readers. Оба бампа предшествуют заморозке v1.0.0, когда ломать формат ещё было приемлемо; после заморозки это невозможно без поколения v4. |
...
offset 24..28   owned_count    u32 LE   (число owned-записей в ЭТОМ chunk'е)
offset 28..32   pool_count     u32 LE   (число записей decoy-пула в ЭТОМ chunk'е)
offset 32..     owned[owned_count] u64 LE каждая (по возрастанию)
                pool[pool_count]   u64 LE каждая (по возрастанию)
```

Оба списка делят один бюджет на chunk:
`owned_count + pool_count <= (PAYLOAD_CAP - 32) / 8`.

Поле `pool_count` и список пула появились вместе с переиспользованием
слотов (DESIGN §9.1), а этот документ описывал 28-байтовый заголовок
дольше, чем тот был верен, — так что реализация, написанная по нему,
разбирала неверно каждый checkpoint, который пишет этот writer
(report9 HV-08).

**Decoy-пул** — набор слотов, которые пространство списало и вправе
выделить снова. Он записывается сюда по причине, которую стоит назвать:
обновление вызывается ростом незачекпоинченного хвоста, а
переиспользование как раз останавливает рост файла — то есть чем лучше
работает переиспользование, тем реже пул попадал бы на диск. Без
отдельного триггера по дрейфу контейнер с идеальным переиспользованием
записал бы checkpoint однажды и потерял бы все освобождённые слоты при
следующем закрытии.

**Писатель, у которого пула нет, не должен записывать это как факт.**
Пул восстановим только из этой цепочки, поэтому сессия, которая её не
читала — постоянное-по-времени открытие берёт полный скан по замыслу,
ведь число чанков быстрого пути и отличает верный пароль от неверного, —
не держит ничего. Запись её пустоты поверх накопленного уничтожает
единственную существующую копию, и каждое последующее открытие начинает
работать только на дозапись, а восстановить пул уже неоткуда. Замерено
на стенде: 39 записанных слотов превратились в 1 (report9 HV-14).

Поэтому писатель читает цепочку, которую собирается заместить, ДО её
затирания и сливает её пул со своим всякий раз, когда своего не
загружал. Именно сливает, а не подменяет: сессия без пула всё равно
освобождает слоты (мусорное дополнение каждого коммита попадает туда).
Обе половины фильтруются по владеемому набору и высокой воде этой эры,
чтобы две половины записи оставались взаимодополняющими: читатель и так
вычитает владеемое из записанного пула, но запись обязана быть верной
сама по себе.
...
`[cp_high_water, slot_count)`. Результат идентичен полному скану, когда
записанный owned-набор полон, что честный писатель гарантирует по
индукции (первый checkpoint всегда пишется из полного скана; каждый
fast-derived наследует полноту). Поэтому forward-secrecy (vacuum
обходит весь `owned_slots`) и `commit_history` сохраняются.

Записанный owned-набор — это **key-authenticated trusted cache**:
читатель отбрасывает слоты, не прошедшие перепроверку, но не может
обнаружить *пропущенный* без полного скана, которого он избегает.
Checkpoint, пропускающий owned-слот — подделать может только держатель
ключа (самовредительство), никогда честный писатель, недостижимо для
противника без ключа — заставит vacuum пропустить этот orphan
(деградация forward-secrecy), но никогда не затронет выигравший
superblock или зафиксированные данные. Это то же доверие, на котором
уже держатся persisted `root_slot` / `root_hash` суперблока.
...
- Запись checkpoint публикует свежую цепочку плюс superblock с
  **увеличенным seq** (тот же `root_slot` / `root_hash`, новый
  ...
  которую заменяет.
- Он также обновляется, когда членство decoy-пула сдвинулось достаточно
  с момента последней записи, независимо от хвоста — почему хвост один
  этого не вытянет, сказано выше.
- Цепочка строится ТОЛЬКО из дописанных слотов. Checkpoint-chunk никогда
  не берётся из пула: слоты заменяемой цепочки попадают в пул, а
  superblock на диске до публикации новой цепочки всё ещё называет
  голову старой — checkpoint-chunk, приземлившийся на эту голову, сделал
  бы падение в промежутке невосстановимым.
...
**Constant-time режимы open не используют fast path вообще.** Абзац
выше защищает его на пути по умолчанию, где скорость и есть цель, а
остаточная утечка — принятый размен. На `open_space_constant_time` это
не переносится: его опубликованный контракт в том, что wall-clock хоста
не выдаёт, какое space — или ни одно — подошло, а «отражает только
working-set самого decoy» и есть тот сигнал, ради устранения которого
этот API существует. Выравнивание по каждому chunk не спрячет разницу,
которую несёт САМО ЧИСЛО просмотренных chunk'ов. Вызывающий
constant-time API уже согласился примерно на двойное время открытия;
вернуть ему скорость значит вернуть и утечку, от которой он платил.
...
| `MAX_NAMESPACES_PER_TX` | `(PAYLOAD_CAP - 2 - 32) / (1 + 1 + 8 + 32)` = 95 | Корней — по одному на пространство имён — в одном CommitPayload; выводится из вместимости payload, а не выбирается. Раньше в этой таблице стоял `MAX_RECORDS_PER_TX` = 100: такой константы в крейте нет и не было, и считала она не то — потолок на пространства имён, а не на записи. В §4.3 всё это время стояло верное число. |
...
| traversal guard (`space/walk.rs`) | visited set + бюджет = число owned-чанков | Дополнение к cap'у глубины: тот ограничивает, как *глубоко* идёт обход, но не как *широко*. Кодирование ничем не требует, чтобы указатели `child_slot` внутри узла были различны, поэтому DAG — все дети одного узла называют один и тот же следующий узел — AEAD-валиден, Merkle-консистентен и стоит `fanout^depth` чтений из горстки чанков. Каждый рекурсивный walker сначала регистрирует слот в guard'е: повторный визит — `"chunk reachable more than once in one tree walk"`, обход, которому нужно больше чтений, чем у пространства есть чанков, — `"tree walk exceeded its traversal budget"` (`Error::Malformed`, либо `Error::IntegrityFailure` внутри `verify_integrity`). |
| диапазоны ключей индекса (только `verify_integrity`) | `[children[i].first_key, children[i+1].first_key)` | Каждый ключ под ребёнком `i` обязан лежать в этом полуинтервале; последний ребёнок наследует верхнюю границу родителя. `LeafNode::decode` проверяет порядок только *внутри* одного листа, поэтому соседние листья могли перекрываться или идти вразнобой при совпадающих хешах — давая записи, до которых `Space::get` не доходит (он спускается бинарным поиском по `first_key`) и которые гейт глобальной сортированности в `flatten_tree` отверг бы на следующем коммите. |
