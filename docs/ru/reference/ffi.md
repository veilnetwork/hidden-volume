...
конструкторы (`create`, `open`), reads (`get`, `count`,
`list_namespaces`, `read_log`, `iter_log_range`, `commit_seq`,
`commit_history`, `stats`, `verify_integrity`) и write
(`commit`). Те же аргументы, та же форма ошибок, та же семантика —
просто `async fn` повсюду.
...
- Async: `AsyncSpaceHandle::create`, `AsyncSpaceHandle::open`.
