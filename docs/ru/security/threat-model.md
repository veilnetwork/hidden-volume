...
**Статус.** Рабочий документ, актуален для крейта v2.6.x. Структура не
меняется между релизами — добавляются только конкретные результаты.
Эта строка называла проект предрелизным ещё долго после выхода v2 — так
выглядит документ, в который никто не возвращался (report16 HV16-L1). Внешний платный аудит (класса Trail of Bits /
...
Однофайловый, зашифрованный, multi-space примитив хранения. Он был
append-only, пока DESIGN §9.1 не снял это: писатель теперь ещё и
переиспользует списанные им слоты, а рядом перерандомизирует столько же
приманок, чтобы на диске это было одним видом события. `Inv-W1` —
нынешняя формулировка того, что писателю дозволено трогать: «только
слот, недостижимый из любого superblock'а, который может выбрать
переоткрытие», а не «только конец файла». Ядро — синхронный, std-only Rust. Опциональная обёртка
...
- Open path выбирает Superblock с max-seq, который AEAD-decrypts,
  ...
  писатель (наибольший слот). Двух разных
...
| **`mmap` и любой локальный процесс, не берущий блокировку** | Cargo-фича `mmap` использует `memmap2::Mmap`, чей контракт требует стабильности байтов на время маппинга. `flock(LOCK_EX)` исключает только тех, кто блокировку просит; усечение файла процессом, который её не просит, даёт SIGBUS и падение процесса — на любой ФС. На NFS/FUSE блокировка вдобавок не работает и для просящих. | См. §4.2 ниже; `mmap` — `cfg(unix)` И opt-in через Cargo feature; mobile / FFI consumers должны держать выключенным |
...
конструированием маппинга, что исключает writer'а, который **просит**
блокировку: он блокируется или получает `Error::Busy`, пока наш
маппинг не дропнется.

**Больше он не исключает никого, ни на какой ФС.** `flock(2)` —
рекомендательная. Процесс того же пользователя, открывший контейнер
**без** блокировки и усёкший его, ничем не связан, и скан касается
страницы, за которой больше нет данных — SIGBUS, аварийное
завершение процесса, на ext4 и APFS ровно так же, как где угодно.
Раздел говорил, что блокировка «удовлетворяет контракт» на локальных
ФС, и относил риск к сетевым монтированиям; такое чтение сообщало
администратору, что его локальное развёртывание безопасно, тогда как
это не так (report17 HV17-L2, вслед за HV16-M1).

С report17 HV17-L3 четыре точки входа помечены `unsafe fn` и несут
это требование в сигнатуре — вызывающий обязан заявить, что оно
выполняется, а не узнавать об этом здесь.

**Где блокировка бесполезна дополнительно — даже против writer'ов,
которые её просят.**
...
через constant-time scan (см. CHANGELOG `[1.2.0]`), так что host-app на FFI —
...
**Митигация заканчивается там, где open возвращает управление (аудит
HV-01).** До этого прохода каждый constant-time-open на writable-
handle перед возвратом запускал `vacuum_orphans` — обход дерева,
чтение каждого невидимого chunk'а среди достижимых, перезапись
каждого orphan'а и fsync. Миллисекунды и записи на диск, и то и
другое пропорционально накопленной истории space'а, и достигается это
**только на success-пути**: неверный пароль возвращается из scan'а до
этой строки. За аккуратными микросекундами equalizer'а шёл сигнал на
несколько порядков крупнее, и наблюдатель за процессом или
файловой системой в момент ввода пароля читал ответ прямо с него.

Constant-time-точки входа больше не делают обслуживания.
[`Space::vacuum_after_open`](../../../crates/hidden-volume/src/space/vacuum.rs)
— та же работа отдельной операцией, `MultiSpace::vacuum_hosted` — её
мультипространственный близнец. **Затирание отложено, а не отменено**
— пока host его не выполнит, прежние версии удалённых или
перезаписанных значений остаются восстановимыми для того, кто позже
получит пароль и старый снимок файла, поэтому host, открывающий
constant-time, обязан куда-то его подключить. Но не в строку после
open'а: та же длительность мгновением позже — всё ещё длительность
разблокировки. Flutter-плагин ставит его на криптографически
случайную задержку из окна (`DeferredVacuumWindow`, по умолчанию
30 с – 5 мин) и снимает при close.
...
| 2026-05-10 | Pass 18 (second-reviewer follow-through) | M1: `commit_tx` больше не Err после durable commit; M2: `verify_integrity` покрывает DataBatch chunks; M3: `atomic_rewrite_under_source_lock` race-window сужен через re-open + inode-preservation check; M4: Android lock-skip precondition задокументирован; M5: v3 format-binding requirement зафиксирован. | CHANGELOG `[2.0.0]` (pass-18 секция) |
...
| 2026-05-28 | 5-pass deep-review series (adversarial-stance / primitive-level / side-channel-surface / format-fuzzing / threat-model-challenge) | 0 critical / 0 high / 0 medium findings. SC-INFO2 закрыт multi-variant TM1-bench'ем (parallel-scan НЕ вымывает сигнал). F-A5 закрыт `MAX_TREE_DEPTH = 3` cap'ом в B+ tree walker'ах — ВПОСЛЕДСТВИИ ЗАМЕНЁН: аудит HV-15 снял эту константу, потому что она заодно служила потолком вместимости пространства имён, и walker'ы теперь выводят предел глубины из числа чанков, которыми пространство владеет (`format.md` §8, tree depth). F-TM1 частично смягчён opt-in `Container::open_space_constant_time`. | `docs/ru/security/audits/*.md` |
