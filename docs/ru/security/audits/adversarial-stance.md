...
- **Verdict.** **Defended.**
- **Severity.** INFO.
- **Follow-up (2026-08-01).** Cap — на **один** batch, а декодер
  `iter_log_*` держал по одному декодированному batch'у на каждый
  различный `batch_slot` страницы, без совокупного предела: N
  указателей на N подделанных batch'ей — это `N × 8,4 МиБ`
  резидентно. N выбирает вызывающий через `limit`, а у `iter_log`
  лимита нет вовсе: страница в 512 — это ~4,3 ГиБ из контейнера в
  пару МиБ. Cap на элемент — не бюджет. Теперь кеш ограничен и по
  байтам, и по числу записей (`MAX_CACHED_BATCH_BYTES` = 8 МиБ,
  `MAX_CACHED_BATCHES` = 64) и вытесняет least-recently-used, а не
  падает: batch всегда можно перечитать и передекодировать, и ни
  один результат не зависит от того, что осталось резидентным. Пик
  декодированных байт за вызов — бюджет плюс один декодируемый
  batch, то есть меньше ~16,4 МиБ при любом `limit`.
...
- **Severity.** INFO.
- **Follow-up (2026-08-01).** Закрыто, и шире самой находки.
  Cap `MAX_TREE_DEPTH = 3` приехал первым (2026-05-28) и
  покрывает cycle. Чего он не покрывает — **DAG**: находка
  рассуждает про cycle, потому что именно cycle ломает цепочку
  *хешей*, но повтор дочернего указателя не ломает ничего —
  `H(B)` одно и то же под каждым родителем, называющим B, так
  что узел, у которого все ~90 детей указывают на один
  следующий узел, AEAD-валиден, Merkle-консистентен и проходит
  `verify_integrity`. На cap'е глубины это `90³ ≈ 7,3 × 10⁵`
  чтений чанков из четырёх различных чанков. Теперь все
  рекурсивные walker'ы делят общий traversal guard
  ([`space/walk.rs`](../../../../crates/hidden-volume/src/space/walk.rs)):
  visited set (чанк, достижимый дважды, — структурный отказ, а
  не работа, которую надо повторить) плюс бюджет, равный числу
  owned-чанков пространства. Так что фраза «не имеют
  visited-set'а» выше более не верна ни для одного walker'а.
  `verify_integrity` дополнительно проверяет диапазоны ключей
  соседних детей — того цепочка хешей тоже выразить не может.
- **Follow-up (2026-08-03, аудит HV-15).** Константы больше нет,
  защита осталась. `MAX_TREE_DEPTH = 3` был заодно потолком
  ёмкости у писателя — namespace не мог держать больше листьев,
  чем помещается под одним корнем (79 записей при значениях по
  2 KiB), поэтому снятие потолка означало, что читатели больше
  не вправе закладываться на глубину. Предел теперь выводится из
  бюджета обхода, который и так есть в `TreeWalk`: каждый уровень
  корректного дерева минимум в `MIN_FULL_INTERNAL_FANOUT` раз шире
  вышестоящего, поэтому обход может спуститься лишь настолько
  глубоко, насколько могли бы быть уложены chunk'и, которыми
  владеет space. На самом большом контейнере, который допускает
  формат (`MAX_OPEN_SCAN_CHUNKS`), это 7 спусков, и предел
  уменьшается вместе с контейнером: почти пустой space отвергает
  цепочку в три спуска. Против находки в её формулировке это
  строго лучше: глубина, которую держатель ключа не может
  оплатить чанками, недостижима, тогда как константа выдавала
  одну и ту же глубину и контейнеру в 4 KiB, и контейнеру в
  64 GiB. Писатель отказывается публиковать дерево, которое
  отвергли бы его же читатели, поэтому две стороны не могут
