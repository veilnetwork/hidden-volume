...
`sync_all()` вернёт `Err`, Tx прерывается. On-disk state может быть:
любой префикс записей durable, post-error записи volatile в
OS-буферах — тот же crash-safety анализ, что выше. Caller НЕ должен
ретраить тот же Tx без предварительного re-open контейнера для
повторного derive state.

**Какую ошибку увидит caller — зависит от того, где сломалось**
(report8 H-09). До Barrier 2 включительно (fsync Commit-чанка) ничего
записанного читателю не достижимо, поэтому такие отказы приезжают как
`Error::Io(_)` и описывают себя точно. Barrier 3 — публикация
Superblock'а — другое дело: `commit_tx` сжигает новый `seq` раньше,
чем первая реплика способна лечь на диск, поэтому отказ там может
оставить на диске seq ВЫШЕ, чем `superblock.seq` у хэндла. Любой
отказ в этом окне — `Error::PublishUncertain(_)`, чьё единственное
сообщение и есть фраза «сначала переоткрой» выше. Раньше там тоже был
`Error::Io(_)`, что читается как «запись не состоялась» и противоречило
совету этого же раздела. Исходная причина сохраняется в
`Space::last_publish_error()`.

Self-heal чекпоинта публикует Superblock тем же путём и отвечает тем
же вариантом. Коммиты после этого НЕ блокируются: следующий `seq`
выводится из метки сжигания, поэтому он перескакивает сожжённый номер,
а не публикует под ним второй payload (аудит HV-01).
