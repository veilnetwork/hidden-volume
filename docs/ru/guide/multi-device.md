...
После каждого успешного commit host-app записывает эру, которую он
только что опубликовал, — ПАРУ `(seq, root_hash)` из последней записи
`space.commit_history_with_roots()` — в место, которое adversary не
может переписать.

**Пару, а не один seq.** Обе ветви развилки считают коммиты одинаково,
поэтому anchor, хранящий только число, не отличает «это то
пространство, которое я оставил» от «это копия, в которую с тех пор
писали»: обе доходят до seq N и обе проходят проверку на членство по
числу. Root — это то, что коммит на самом деле опубликовал, и другая
ветвь, дошедшая до того же seq, опубликовала другое. До 2.3 root
наружу не выводился вовсе, поэтому сделать эту проверку обоснованно не
мог ни один хост — а раздел выше описывал fingerprint поверх «Superblock
seq и root_hash байтов, оба уже хранятся в state'е `Space`», то есть
API, которого не было (report22 HV-FORK-SEQ).
...
> **О версии.** Сравнение ПАРЫ различает ветки только если корень зависит от
> того, что записано. До hidden-volume 2.5.0 индекс журнального пространства
> хранил лишь СЛОТ пакета, поэтому две копии одного контейнера, дописавшие
> записи одинаковой формы, давали одинаковую пару `(seq, root_hash)` при разном
> содержимом — ветвление читалось как чистое продолжение (report24 HV24-01).
> С 2.5.0 значение индекса несёт ещё и хеш содержимого пакета.
>
> Цена односторонняя: контейнер, записанный 2.5.0 и новее, старые сборки читают
> как «пространство не является журналом». Новые читают обе раскладки.

На `Container::open_space`:

1. Прочитать внешний anchor `(anchor_seq, anchor_root)`.
...
3. Сравнить:
   - `current_seq < anchor_seq` → **rollback (откат)**. Отказаться
     ...
     anchored-данные).
   - `current_seq >= anchor_seq` И пара `(anchor_seq, anchor_root)`
     есть в `space.commit_history_with_roots()` → **clean
     continuation**. Принимаем. Сверять ПАРУ: наличие одного лишь
     `anchor_seq` — это то, что могут сказать обе ветви развилки.
   - `current_seq - anchor_seq > hidden_volume::ANCHOR_HORIZON` →
     **вне окна**. Anchor старше окна, которое файл хранит; его
     отсутствие в `commit_history()` не говорит НИЧЕГО ни в одну
     сторону. Переанкорьтесь от текущего состояния, а если у хоста
     есть другой способ проверки (серверный счётчик, второе
     устройство) — используйте его. Это НЕ развилка.
   - `current_seq >= anchor_seq`, внутри горизонта, И пары НЕТ в
     `space.commit_history_with_roots()` → **fork**. Timeline файла
     расходится с вашим anchor. Считайте враждебным. Это покрывает
     случай, который проверка по seq не могла: анкоренное ЧИСЛО на
     месте, а эра, которую оно называет, — чужая.

Тест на membership — это часть, отличающая
...
**Случай «вне окна» новый, и порядок трёх проверок важен.** Раньше
anchors хранились вечно, поэтому отсутствие означало развилку и ничего
больше. Вечное хранение означало и то, что каждый коммит оставлял два
чанка, которые никто никогда не собирал: контейнер, переписывающий один
ключ, рос примерно на 17 КБ на коммит без выхода на плато.
`ANCHOR_HORIZON` ограничивает это 1024 эрами, и цена ровно такая:
anchor дальше горизонта на диске больше не с чем сравнивать.

Хост, который проверит membership, не проверив сперва дальность,
объявит враждебным каждое давно не подключавшееся устройство. Если ваши
устройства бывают офлайн дольше, чем `ANCHOR_HORIZON` коммитов, —
анкорьтесь чаще либо держите на этот разрыв второй источник anchor.
