...
> **⚠ ПОТЕРЯ ДАННЫХ BY DESIGN.** Spaces, НЕ упомянутые в маппинге,
> **молча и безвозвратно сбрасываются** (та же деструктивная
> семантика, что у `compact_known`). Пустой / неполный список
> паролей сбросит *каждый* непереисленный space. Это свойство
> deniability: библиотека не может перечислить deniable spaces,
> поэтому не способна обнаружить или предупредить о потере —
> host-app ОБЯЗАН подтвердить полноту набора паролей перед вызовом.
> Чтобы сохранить space, чей пароль не ротируется, перечислите его
> как no-op-пару `(p, p)`.

Механика: пишет свежий контейнер в sibling-temp с именем
`.{stem}.hv-rotate.{16hex}.tmp`, затем делает атомарный rename
поверх `path` под source `LOCK_EX` с `fsync` родительской
директории. На любой ошибке ДО rename временный файл удаляется, а
исходный `path` остаётся нетронутым.

Уточнение не украшение, и на этой странице его не было. `rename(2)` —
точка невозврата, и ПОСЛЕ неё сообщаются четыре исхода; в каждом
старого контейнера по этому пути уже нет, поэтому ни один не является
ошибкой, которую можно повторить против него:

| Исход | Что верно |
|---|---|
| `RenameVisibleDurabilityUncertain` | Перезапись применена; переживёт ли запись каталога сбой — неизвестно. |
| `RenameVisibleContentUnverified` | По пути лежит файл, который этот вызов не писал. |
| `RenameVisibleAliasesNotRevoked(n)` | Перезапись применена по этому имени; прежний файл достижим ещё по `n` жёстким ссылкам. |
| `RenameVisibleAliasesUnknown` | Перезапись применена; платформа не смогла сказать, остались ли другие имена у прежнего файла. Это не то же самое, что «их нет». |
| `RenameVisibleAliasesAndDurabilityUncertain` | Оба предыдущих сразу. Это независимые факты, и оба доезжают: средства разные. |

И один, который в эту группу НЕ входит — вынесен отдельно, потому что
раньше стоял в той же таблице под заголовком, утверждавшим, что всё в ней
уже произошло (report17 HV17-L2):

| Исход | Что верно |
|---|---|
| `SourceIsNotARegularFile` | Отказ ДО открытия чего-либо — символьная ссылка или не-файл, где rename заменил бы ИМЯ, оставив контейнер, на который оно указывает, прежним. **Ничего не выполнялось**, прежний контейнер нетронут. |

После смены пароля каждый исход ПЕРВОЙ таблицы означает, что НОВЫЙ пароль
действует по этому пути; повторять со старым — неверно. Вторая таблица —
противоположный случай: не записано ничего, и контейнер по-прежнему
открывается старым паролем. Cancellable-вариант
...
**Q: Что произойдёт, если сделать `delete` отсутствующего ключа?**
A: Это **намеренно НЕ no-op**: удаление отсутствующего ключа всё
равно пишет commit и продвигает sequence number. Это by design, а
не пустая запись. Если бы delete отсутствующего ключа делал
short-circuit (пропускал commit), то *вырос ли файл / сдвинулся ли
`commit_seq`* наблюдаемо раскрывало бы «этот ключ существовал»
против «не существовал» multi-snapshot-противнику (T2'),
следящему за контейнером — оракул существования ключа. Безусловная
запись commit делает delete **намеренным tombstone-anchor**:
удаления существующего и отсутствующего ключа неотличимы снаружи.
Host-app'ам не следует трактовать лишний commit как баг или
пытаться его устранить.
...
A: Да. v1.0.0 вышел и заморозил on-disk формат на поколении v3.
Reader отказывает любому `format_version != 3`. Любое будущее
изменение раскладки требует нового поколения (v4) плюс инструмент
миграции; оно не будет молча переинтерпретировать v3-файл. См.
`docs/ru/reference/format.md` §7 (cross-version policy).
...
| Миграция между поколениями (export/re-import; нет in-place vN → vM) | `docs/ru/guide/migration.md` |
