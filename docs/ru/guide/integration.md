...
| `SourceIsNotARegularFile` | Отказ ДО открытия чего-либо — символьная ссылка или не-файл, где rename заменил бы ИМЯ, оставив контейнер, на который оно указывает, прежним. |

После смены пароля первые три означают, что НОВЫЙ пароль действует по
этому пути. Повторять со старым — неверно. Cancellable-вариант
