...
**Статус.** Формат **заморожен на v3** — с релиза v1.0.0
(2026-05-28). До этого поколение формата бампилось дважды
(v1 → v2 в audit pass 13, v2 → v3 — 2026-05-28); эти bump-ы
**намеренно breaking**, in-place migration-tool не поставляется.
Cross-version переходы идут через export-and-reimport.

**v2.0.0 (2026-08-12) НЕ вводит новое поколение.** Major bump взят
за публичные Rust + FFI + Dart API; `PARAMS_VERSION` остаётся 3, и
контейнер, записанный 1.2.x, открывается под 2.0.0 без изменений —
поэтому **migration tool не требовался и не поставляется**. Одна
оговорка работает в обратную сторону и не относится к формату:
writer 2.0.0 может выдать B+-деревья глубже, чем `MAX_TREE_DEPTH = 3`,
который проверял каждый walker 1.2.x, поэтому контейнер, в который
писал 2.0.0, может оказаться нечитаемым для сборки 1.x. In-place
downgrade отсутствует; рецепт export-and-reimport ниже не зависит от
поколения и является запасным вариантом, если downgrade всё же нужен.
...
| 2026-05-28 | v1.0.0 — формат заморожен на v3 | `CHANGELOG.md`, запись 1.0.0 |
| 2026-08-12 | **v2.0.0 — major bump БЕЗ смены поколения.** Сломаны API; `PARAMS_VERSION` остаётся 3; migration tool не нужен. 1.x не прочитает глубокие деревья, которые пишет 2.0.0. | `CHANGELOG.md`, запись 2.0.0 |
| post-2.0 (TBD) | Первый полноценный migration tool (только если появится поколение v4) | Этот документ, расширенный |
