...
**Читать вместе с ratio ниже, никогда по отдельности** (report10
HV-04). Низкое отношение при большом пуле — это здорово
перерабатывающий контейнер, которому компактизация не нужна: решение
по одному только ratio перепишет весь файл и провернёт
`container_id` впустую. Низкое отношение при пуле около нуля — как
раз та форма, которой L5 нужен. Пересекает FFI как
`StatsInfo.reusable_slot_count`; до report10 HV-04 не пересекало
вовсе, так что у host-app на других языках была лишь та половина
пары, которая не может решить сама.
