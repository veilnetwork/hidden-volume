...
try {
  ...
  switch (space.outcomeOf(op.id)) {
    ...
    case HvOpFailed():    /* воркер ответил отказом, ретрай безопасен */
...
что воркер был жив, выполнил вызов и ядро отказало: ничего не
закоммичено, ретрай безопасен. `HvOpIndeterminate` значит, что воркер
*умер* под вызовом, — а изолят может умереть уже ПОСЛЕ того, как
нативная фиксация легла на диск, и ДО того, как ушёл ответ, поэтому со
стороны Dart неизвестно, применилась запись или нет. Переоткрыть
контейнер и посмотреть.
