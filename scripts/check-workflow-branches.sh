...
#
# `ci-branch.yml` describes itself as the fast feedback signal — "every commit
# gets a fmt/clippy/test/audit sanity check within ~3-5 minutes, so the drift
# audit pass 19 caught only at tag-time would have been caught at push-time".
# It was aimed at `master`. The default branch here is `main`. From the day it
# was written until 2026-08-30 it fired exactly never, and nothing said so: a
# workflow with no matching pushes is indistinguishable, in the Actions tab,
# from one that simply has nothing to do.
#
