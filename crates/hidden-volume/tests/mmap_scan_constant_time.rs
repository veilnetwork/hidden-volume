//! Behavioral equivalence: mmap-scan CT companion ↔ sequential.
//!
//! `Container::open_space_mmap_constant_time` (v1.0) combines the
//! zero-allocation mmap read path with the per-chunk ChaCha20
//! timing equalizer. The recovered `Space` MUST be observationally
//! identical to what the default sequential scan produces — only
//! the per-chunk timing shape differs. Mirrors
//! `tests/constant_time_scan.rs` for the mmap path.
...
#[test]
fn mmap_constant_time_recovers_same_state_as_sequential() {
    ...
    assert_eq!(
        seq_seq, ct_seq,
        "commit_seq must match across mmap/sequential CT modes"
    );
    assert_eq!(
        seq_owned, ct_owned,
        "owned_chunk_count must match across mmap/sequential CT modes"
    );
    assert_eq!(
        seq_count, ct_count,
        "namespace entry count must match across mmap/sequential CT modes"
    );
}

#[test]
fn mmap_constant_time_wrong_password_returns_auth_failed() {
    let path = scratch_path();
    {
        let mut c = Container::create_with_options(&path, fast_options(3)).unwrap();
